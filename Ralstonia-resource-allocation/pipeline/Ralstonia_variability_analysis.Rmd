...
## Overview on summarized protein utilization
...
plot_utilization_bar <- df_utilization %>%
  ...
  xyplot(mol_fraction ~ factor(condition), .,
    ...
    xlab = "", ylab = "mol fraction", ylim = c(0, 1.4),
    scales = list(alternating = FALSE), 
    col = grey(c(0.4, 0.5, 0.65, 0.8)),
    panel = function(x, y, subscripts, ...) {
      panel.grid(h = -1, v = -1, col = grey(0.9))
      panel.barchart(x, y, horizontal = FALSE, subscripts = subscripts, ...)
      panel.key(..., corner = c(0.02, 0.98), points = FALSE)
      panel.text(x = as.numeric(x), rep(c(0.3, 0.55, 0.8, 1.04), 4), labels = round(y, 2), cex = 0.7)
    }
  )
...
```{r}
png("../figures/figure_protein_utilization.png", width = 1600, height = 2200, res = 200)
