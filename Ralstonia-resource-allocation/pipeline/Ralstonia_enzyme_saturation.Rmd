...
Ralstonia_eutropha <- Ralstonia_eutropha %>%
  ...
  select(uniprot, locus_tag, protein, condition, substrate, substrate_uptake_rate,
    growth_rate, mol_fraction, mean_mass_fraction, sd_massfraction, mass_g_per_gDCW, COG_Process) %>%
  
  # transform SD mass fraction into SD g/gDCW same way as was done for mass
  mutate(sd_g_per_gDCW = sd_massfraction * 0.65)
```
...
df_prot_comp <- df_model_reactions %>%
  ...
  group_by(condition) %>% mutate(
    ...
    mass_g_gDCW = mass_g_per_gDCW/n_reactions,
    sd_mass_g_gDCW = sd_massfraction/n_reactions
  ) %>%
  ...
  summarize(
    ...
    sd_mass_g_gDCW = sum(sd_mass_g_gDCW, na.rm = TRUE)
  ) %>%
...
Now we perform a test. We check if all protein abundances allocated to reactions sum to a reasonable value as we would expect. This value would be the total *enzymatic* protein mass in g/gDCW, per condition, and could reach up to 0.2 g/gDCW for the simulations, and higher for the actual data (includes additional protein quantified in experiment, but not carrying flux in model simulations).
...
# Proteins 'over-predicted' by more than 10x
...
xyplot(log10(predicted_mass_g_gDCW) ~ log10(mass_g_gDCW) | factor(growth_rate) * substrate, 
  df_prot_comp, labels = df_prot_comp$reaction_id, 
  groups = substrate,
  ...
  panel = function(x, y, ...) {
    ...
    panel.xyplot(x, y, ...)
    #panel.directlabel(x, y, subscripts = subscripts, y_boundary = c(-2.2, 0), ...)
  }
)
```
...
```{r, echo = FALSE}
# A generalized plotting function with error bars
xyplot_errbars <- function(data, variables, groups = NULL,
  error_col = NULL, regline = FALSE, key = TRUE, 
  xlab = NA, ylab = NA, xlim = NULL, ylim = NULL, ...) {
  ...
  # plotting symbol and error columns custom for each variable
  plot_symbols <- rep(c(19, 1, 20, 2, 21, 3), length.out = length(variables))
  error_col <- rep(error_col, length.out = length(variables))
  
  # if groups is NULL, add a single pseudo group
  if (is.null(groups)) {
    data$nogroups <- rep("no groups", nrow(data))
    groups = "nogroups"; key = FALSE
  }
  
  # plot each layer individually
  plots <- lapply(variables, function(vars) {
    
    layer <- which(variables %in% vars)
    xyplot(as.formula(vars), data, 
      groups = get(groups),
      par.settings = custom.colorblind(),
      error_margin = {if (!is.na(error_col[layer])) data[[error_col[layer]]] else NULL},
      scales = scaleoptions, as.table = TRUE,
      between = list(x = 0.5, y = 0.5), 
      pch = plot_symbols[layer],
      xlab = xlab, ylab = ylab,
      col = custom.colorblind()$superpose.line$col,
      panel = function(x, y, subscripts, error_margin, ...) {
        if (layer == 1) {
          panel.grid(h = -1, v = -1, col = grey(0.9))
          if (key) {panel.key(..., cex = 0.7)}}
        panel.superpose(x, y, subscripts, error_margin, ...)},
      panel.groups = function(x, y, subscripts, error_margin, ...) {
        panel.xyplot(x, y, ...)
        if (!is.null(error_margin)) {
          stdev = error_margin[subscripts]
          panel.segments(x0 = x, y0 = y, x1 = x, y1 = y + stdev, ...)
          panel.segments(x0 = x, y0 = y, x1 = x, y1 = y - stdev, ...)}
        if (regline) {panel.lmline(x, y, ...)}
      }
    )
  })
  ...
  if (length(plots) > 1) {
    for (i in 2:length(plots)) {
      final_plot <- final_plot + as.layer(plots[[i]])
    }
  }
...
df_machinery %>%
  ...
  group_by(machine, substrate, growth_rate) %>%
  summarize(
    mass_g_gDCW = sum(mass_g_per_gDCW, na.rm = TRUE),
    sd_g_gDCW = sum(sd_g_per_gDCW, na.rm = TRUE),
    predicted_mass_g_gDCW = sum(predicted_mass_g_gDCW, na.rm = TRUE)
  ) %>%
...
xyplot_errbars(df_prot_comp %>% filter(reaction_id %in% calvin),
  variables = c("mass_g_gDCW ~ growth_rate | reaction_id * substrate",
    "predicted_mass_g_gDCW ~ growth_rate | reaction_id * substrate"),
  groups = "substrate", error_col = c("sd_mass_g_gDCW", NA), 
  key = FALSE, ylim = c(-0.002, 0.032))

# Plot TCA cycle reactions
xyplot_errbars(df_prot_comp %>% filter(reaction_id %in% tca),
  variables = c("mass_g_gDCW ~ growth_rate | reaction_id * substrate",
    "predicted_mass_g_gDCW ~ growth_rate | reaction_id * substrate"),
  groups = "substrate", error_col = c("sd_mass_g_gDCW", NA), 
  key = FALSE, ylim = c(-5*10^-4, 6*10^-3))
```
