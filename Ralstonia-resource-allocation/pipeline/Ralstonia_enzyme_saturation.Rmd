...
print(plot_calvin, position = c(0, 0, 0.84, 1), more = TRUE)
print(plot_calvin_util, position = c(0.81, 0, 1, 1.))
grid::grid.text(c("A", "B"), x = c(0.02, 0.81), y = c(0.97,0.97))
```

```{r, include = FALSE}
svg("../figures/figure_calvin_util.svg", width = 7.5, height = 5.5)
print(plot_calvin, position = c(0, 0, 0.84, 1), more = TRUE)
print(plot_calvin_util, position = c(0.81, 0, 1, 1.))
grid::grid.text(c("A", "B"), x = c(0.02, 0.81), y = c(0.97,0.97))
dev.off()
...
plot_underutil_by_cond <- df_prot_comp %>% group_by(reaction_id) %>%
  ...
  xyplot(percent_utilization ~ factor(growth_rate) | substrate, .,
    par.settings = custom.colorblind(),
    xlab = expression(µ*" [h"^-1*"]"),
    ylab = "utilization", ylim = c(0, 1),
    scales = list(alternating = FALSE), as.table = TRUE,
...
```{r, , fig.width = 7.3, fig.height = 3.3, message = FALSE}
plot_underutil_by_pw <- df_prot_comp %>%
  ...
  xyplot(percent_utilization ~ model_group_basic %>% factor(., unique(.)[c(2,6,4,1,3,5)]) | substrate, .,
    par.settings = custom.colorblind(),
    xlab = "", ylab = "utilization", ylim = c(0, 1.3),
...
svg("../figures/figure_pathway_util.svg", width = 6.5, height = 4.5)
print(plot_underutil_by_cond, position = c(0, 0.49, 1, 1), more = TRUE)
print(plot_underutil_by_pw, position = c(0, 0, 1, 0.58))
dev.off()
```
