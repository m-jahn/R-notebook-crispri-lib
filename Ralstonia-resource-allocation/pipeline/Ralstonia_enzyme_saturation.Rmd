...
One completely supervised strategy could for example entail the summary of each reactions's protein abundance over growth rate by fitting a linear regression model. One could then coarsely cluster (or hand pick) groups of proteins that will

- increase in simulation, increase in experiment
- increase in simulation, decrease/are constant in experiment
- remain constant in simulation, decrease in experiment
- ...

One additional layer of complexity is that this analysis can be performed for all four available substrate limitations. The individual trends for one or the other limitation might be different for the same protein. We can group the data by reaction and substrate limitation and fit a model over five growth rate and four replicates (20 data points). The slope, intercept and ANOVA p-value can be extracted for experimental protein abundance. For model simulations, the slope is sufficient as intercept is likely zero, and p-value extremely low due to the inherent linearity of predictions with growth rate.
...
df_clusters <- df_prot_comp %>%
  ...
  summarize(
    average_mass_g_gDCW = mean(mass_g_gDCW, na.rm = TRUE),
  ...
  mutate(cluster = case_when(
    lin_reg_slope >  0 & model_slope >  0 ~ "A",
    lin_reg_slope <= 0 & model_slope >  0 ~ "B",
    lin_reg_slope <= 0 & model_slope <= 0 ~ "C",
    lin_reg_slope >  0 & model_slope <= 0 ~ "D"
  ))

# number of reactions and total protein mass per 'cluster'
df_clusters %>% ungroup %>%
  arrange(desc(average_mass_g_gDCW)) %>%
  
  xyplot(average_mass_g_gDCW ~ factor(cluster) | substrate, .,
    par.settings = custom.colorblind(),
    groups = factor(reaction_id, unique(reaction_id)), col = grey(0.6),
    xlab = "group", ylab = bquote("average mass g gDCW"^-1),
    ylim = c(0, 0.2), border = grey(0.85),
    scales = list(alternating = FALSE), as.table = TRUE,
    between = list(x = 0.5, y = 0.5), 
    ewidth = 0.15, beside = TRUE, lwd = 0.5,
    panel = function(x, y, subscripts, groups, ...) {
      panel.grid(h = -1, v = -1, col = grey(0.9))
      panel.barchart(x, y, horizontal = FALSE, stack = TRUE,
        groups = groups, subscripts = subscripts, ...)
      panel.text(x = seq_along(unique(x)), y = 0.015 + tapply(y, x, sum), 
        labels = table(x), cex = 0.7)
    }
  )
```
