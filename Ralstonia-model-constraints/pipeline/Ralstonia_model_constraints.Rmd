...
**Determine enzyme abundance for a standard condition.** Import Ralstonia mass fraction for the standard condition, growth on fructose without nitrogen limitation, fixed growth rate of µ = 0.25 h<sup>-1</sup>, substrate uptake rate qS = 3.03 mmol gDCW<sup>-1</sup> h<sup>-1</sup>.
...
# export to csv
write_csv(df_flux_protein, "../data/protein_concentration.csv")
```

----------

**Determine reaction fluxes for same condition.** Several approaches were tested to obtain minimal and maximal fluxes per enzyme. The best approach turned out to be flux sampling with the additional constraint of reaching at least 95% of the maximum growth rate found by FBA. Flux sampling was performed using COBRApy, 100 simulations with standard settings.

```{r, message = FALSE, fig.width = 7, fig.height = 5}
# import flux sampling results for the highest tested substrate uptake rate,
# qS_fructose = 3.03 mmol/gDCW*h (unlimited NH4+). Flux sampling confined to >= 95% 
# of maximum possible growth rate
df_sampling <- read_csv("../data/FVA_fru_3.03_nh4_10.0.csv") %>%
  select(-1) %>% gather %>% rename(reaction_id = key) %>%
  
  group_by(reaction_id) %>% summarize(
    flux_mmol_gDCW_h = median(value), 
    sd_flux = sd(value), 
    min_flux = min(value), 
    max_flux = max(value),
...
# plot results; we can see that there is a threshold of standard deviation ~ 1
# above which variation gets very high for spurious reactions. These are artificial cycles.
plot_sampling <- lapply(list(A = c(-10, 250), B = c(-0.5, 2)), function(ylim) {
  xyplot(sd_flux ~ seq_along(sd_flux),
    filter(df_sampling, sd_flux != 0), xlab = NULL,
    par.settings = custom.lattice, ylim = ylim,
    labels = filter(df_sampling, sd_flux != 0) %>% pull(reaction_id),
    panel = function(x, y, subscripts, ...) {
      ...
      if (ylim[2] == 2) {
        panel.directlabel(x, y, subscripts = subscripts, 
          y_boundary = c(0.2, 1), draw_box = TRUE, 
          box_line = TRUE, cex = 0.6, ...)
      }
    }
  )
})
...
```{r, message = FALSE}
# the following reactions take part in artif. cycles and have to be constrained separately
cycles <- c("ASPt", "DHFR2p", "DHFR2", "ABUTt", "GLUABUTt7", "PTAr", "URAtr",
            "ALRTa", "ALRTap", "ALRTg", "ALRTgp", "ALCDgl", "ASPO1", "CYTDt2",
            "CYTDtr", "GLUDy", "GLUtr", "INSt2", "ISOVC", "NA1t", "ACKr", 
            "G3PD2", "GCLDH", "GL3PD", "ALCD19", "GLYCTO1p", "NDPK1", "ACOADH3", 
            "NUDPK10", "3HBCD", "ENCOAH1", "HACD1", "HPYRI", "HPYRRy")

# process flux distribution, e.g. by removing extremely high fluxes
df_sampling <- df_sampling %>%
  
  # manual adjustment of reactions that the model   should use (or not),
  filter(!reaction_id %in% c("Biomass", "Maintenance", "Protein", "Carbohydrate", 
    "Phospholipid", "	Cofactors_and_vitamins")) %>%
  
  # add an error margin to the sampled min and max fluxes, to help the solver
  # find a feasible solution  
  mutate(
    min_flux = min_flux-(abs(min_flux)*0.1), 
    max_flux = max_flux+(abs(max_flux)*0.1)
  ) %>%
  
  # replace unrealistic fluxes with a default minimal flux range
  mutate(
    min_flux = if_else(sd_flux > 1, -1.5, min_flux),
    max_flux = if_else(sd_flux > 1,  1.5, max_flux),
    min_flux = replace(min_flux, reaction_id %in% cycles, -0.01),
    max_flux = replace(max_flux, reaction_id %in% cycles, 0.01)
  ) %>%
...
# export to csv
write_csv(df_sampling, "../data/model_flux_sampling.csv")
```


```{r, include = FALSE}
# export also to RBA folder directly
write_csv(df_flux_protein, paste0(RBA_dir, "../calibration/protein_concentration.csv"))
write_csv(df_sampling, paste0(RBA_dir, "../calibration/model_flux_sampling.csv"))
```
