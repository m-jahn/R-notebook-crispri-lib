...
```{r, fig.width = 7, fig.height = 2.8, message = FALSE}
plot_prot_comp <- xyplot(prot_per_compartment ~ growthrate | condition, 
  df_prot_per_comp %>%
    summarize(prot_per_compartment = sum(mol_fraction, na.rm = TRUE)),
  groups = location, ylim = c(-0.15, 1.15),
  par.settings = custom.colorblind(), cex = 0.6,
...
plot_prot_NE <- xyplot(ne_prot_per_compartment ~ growthrate | condition,
  df_ne_prot_per_comp,
  groups = location, ylim = c(-0.15, 1.15),
  par.settings = custom.colorblind(),
  scales = list(alternating = FALSE), layout = c(4,1),
  between = list(x = 0.5, y = 0.5), cex = 0.6,
  xlab = expression("µ [h"^-1*"]"),
  ylab = "protein mass fraction",
  panel = function(x, y, ...) {
    panel.grid(h = -1, v = -1, col = grey(0.9))
...
print(plot_kapp_dist[[1]], position = c(0, 0.66, 1, 1), more = TRUE)
print(plot_kapp_dist[[2]], position = c(0, 0.46, 1, 0.70), more = TRUE)
print(plot_prot_comp, position = c(0, 0.22, 1, 0.49), more = TRUE)
print(plot_prot_NE, position = c(0, 0, 1, 0.27))
```
