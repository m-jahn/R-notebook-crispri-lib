...
# Note: these are not part of the gitbuh repository
...
The second approach of estimating k<sub>app</sub> values is to perform an estimation based on in-house measurements instead of using published k<sub>cat</sub> values from literature or databases. The k<sub>app</sub> constants are actually a collective parameter, a helper construct to link enzyme fluxes to enzyme abundance, so that flux v = k<sub>app</sub> * [E]. The k<sub>app</sub> includes also the saturation s of the enzyme, so that k<sub>app</sub> ~ k<sub>cat</sub> * s. The saturation is between 0 and 1 so that k<sub>app</sub> becomes only lower or equal to k<sub>cat</sub>
...
df_sampling <- lapply(list.files("../data/input/", pattern = "^FSA", full.names = TRUE), 
  ...
  mutate(condition = recode(condition, `1` = "formate", `2` = "ammonium", `3` = "fructose", `4` = "succinate")) %>%
  select(-2) %>% gather(key = "reaction_id", value = "flux", -condition) %>%
  ...
  left_join(lapply(list.files("../data/input/", pattern = "^FBA", full.names = TRUE), 
    read_csv) %>% bind_rows)
...
## Combining estimated kapp values to global consensus kapp

The purpose of this section is to merge different kapp estimations from different conditions to obtain a **consensus kapp estimation**. The reawsoning here is that kapp is estimated from dividing the (sampled or measured) metabolic flux through an enzyme by its abundance, providing an estimate of enzyme efficiency. This efficiency depends also on the saturation of the enzyme. Estimation of kapp is therefore performed for highest available growth rates/fluxes, and four different conditions, in order to obtain enzyme efficiencies at the possiby highest saturation among the tested conditions.
...
### Import kapp estimation from `RBA estim`

```{r, message = FALSE}
# data directory
kapp_calib_dir <- "~/Documents/SciLifeLab/Resources/Models/Bacterial-RBA-models/Ralstonia-eutropha-H16/calibration/"
...
### Manual adjustment of kapp for selected reactions

We import a table with correction factors for kapps that have been estimated extremely wrong. This set of kapps was determined by comparing the correlation between predicted and measured protein abundance. Proteins that are more than 10 fold away from measured abundance are candidates for manual correction. Other reactions that need correction are kapps missing for important transporters. These are also included in the following section.
...
write_tsv(df_kapp_export, paste0(kapp_calib_dir, "kapp_consensus.csv"), col_names = FALSE)
```


```{r, include = FALSE, message = FALSE}
# export figures for kapp determination
png("../figures/kapp_estimation.png", width = 1000, height = 1600, res = 140)
print(plot_kapp_dist[[2]], position = c(0, 0.74, 1, 1), more = TRUE)
print(plot_kapp_dist[[1]], position = c(0, 0.42, 1, 0.77), more = TRUE)
print(plot_kapp_minmax[[1]], position = c(0, 0.2, 1, 0.45), more = TRUE)
print(plot_kapp_minmax[[2]], position = c(0, 0, 1, 0.25))
dev.off()
```
