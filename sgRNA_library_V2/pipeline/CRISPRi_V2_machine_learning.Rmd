...
spread_list_col <- function(
  df, target_col, id_col, sep = "; ", n_min = 5
) {
  select(df, all_of(c(target_col, id_col))) %>%
  mutate(dummy = 1) %>%
  separate_rows({{target_col}}, sep = sep) %>%
  group_by(across(target_col)) %>% filter(n() >= n_min) %>%
...
# spread GO terms
#df_uniprot <- spread_list_col(df_uniprot, "gene_ontology_ids", "locus")

# select final list of features for classification
df_uniprot <- select(df_uniprot, 
  !c(entry, gene_names, `gene_names__(primary_)`, gene_ontology_ids, #`NA`,
  protein_names, ec_number, ec_level_2, ec_level_3, chebi_ids, rhea_id, pfam))
```
...
Extract labels that are highly similar to each other, here by clustering pathways that have more than 50% of genes in common (in other words, a binary distance metric <= 0.5).
...
paste("removed highly similar pathways: ", paste(list_kegg_overlap, collapse = ", "))
...
```{r}
# many columns have low variance
...
fit_summary <- summary(fit_gbm, plot = FALSE)
fit_summary %>% arrange(desc(rel.inf)) %>% slice(1:20) %>%
...
confusionMatrix(
    data = factor(classes_pred, unique(classes_val)),
    reference = factor(classes_val, unique(classes_val))
  ) %>% as.matrix %>% t %>% 
...
# we can also predict the probablity of a gene belong to each class
predict(fit_gbm, newdata = df_train_val, type = "prob") %>%
  head
```
We can look at best predictions in more detail. This table summarizes the classes/labels where the model showed the best predictive performance.
...
df_test_result %>% group_by(kegg_pathway_id) %>%
  ...
  arrange(desc(accuracy), desc(n_genes)) %>%
  filter(accuracy > 0)
```

We can also look at the probability of each gene to be associated with each label.
