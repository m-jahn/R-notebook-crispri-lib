...
list_zerovar_cols <- nearZeroVar(df_train_dummy, freqCut = 99/1, uniqueCut = 1)
df_train_dummy <- df_train_dummy %>% select(-all_of(list_zerovar_cols))
...
Training a neural network usually means to vary (optimize) the weight parameter $w$ for each node such that the cost (difference between expected output and obtained output) is minimized. Here we use the `mlpKerasDropout` function from package `keras`, with default tuning parameters.

```{r, warning = FALSE, results = 'hide'}
# look up model's tuning parameters
modelLookup('mlpKerasDropout')
...
Another type of ML algorithm is a support vector machine, that is quite different to tree-based or neural network based algorithms. A SVM is a supervised machine learning algorithm for classification and regression problems. The SVM algorithm tries to reduce the number of dimensions of the data and finds boundaries between different 'clusters' of data points. The shape of these boundaries is determined by the **kernel function**. It can be linear in the most simple case, or a radial, polynomial, or other type of (non-linear) function. In the simples case, a a classification problem with 2 classes, characterized by two different variables, the boundary between the observations is a line. For three feature variables it is a plane, for n variables it is an **n-dimensional hyperplane**. The boundary between classes is characterized by having the **maximum margin towards the support vectors**, that is, the set of points representing the edge/boundary of a point cluster belonging to a class.
...
plot(fit_gbm, metric = "Accuracy", plotType = "level",
  scales = list(x = list(rot = 90)), par.settings = caretTheme()
)
```

A metric that is very interesting is the contribution of each variable to the overall discriminatory power of the model. We can calculate the relative influence of each variable/feature for the final models and plot it. For the MLP and SVM models, the variable importance is not readily available.
...
#classes_prob_svm <- predict(fit_svm, newdata = df_train_val, type = "prob")
...
df_validation <- df_training %>%
  ...
  mutate(probability = lapply(
    list(classes_prob_gbm, classes_prob_raf, classes_prob_mlp, classes_prob_mlp),
    function(x) apply(x, 1, max)) %>% unlist) %>%
...
plot_prob_correct <- df_summary %>%
  ...
  ggplot(aes(x = as.numeric(kegg_pathway_id), y = value, fill = model)) +
    geom_col(width = 1) +
    labs(title = "performance metrics for classes with corrrect predictions", x = "", y = "") +
    facet_grid(model ~ factor(metric, unique(metric))) +
