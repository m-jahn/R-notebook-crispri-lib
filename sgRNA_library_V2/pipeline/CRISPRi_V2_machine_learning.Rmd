...
A metric that is very interesting is the contribution of each variable to the overall discriminatory power of the model. We can calculate the relative influence of each variable/feature for the final model and plot it. For the MLP model, the variable importance is not readily available.
...
df_validation <- df_training %>%
  ...
  pivot_longer(cols = c("GBM", "RAF", "MLP"),
    names_to = "model", values_to = "prediction") %>%
  arrange(factor(model, c("GBM", "RAF", "MLP"))) %>%
  mutate(probability = lapply(
    list(classes_prob_gbm, classes_prob_raf, classes_prob_mlp),
  ...
  ungroup %>% mutate(model = factor(model, c("GBM", "RAF", "MLP")))
```
