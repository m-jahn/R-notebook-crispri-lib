...
df_ncRNA %>% group_by(ncRNA_type) %>%
  ...
  select(sgRNA_target, ncRNA_type, score) %>%
  summarize(n_targets = length(unique(sgRNA_target)), sig = sum(score >= 4),
    non_sig = sum(score < 4), percent = sum(score >= 4)/n()*100)
```
...
plot_asRNA_xy <- df_ncRNA_select %>% filter(ncRNA_type == "asRNA") %>%
  ...
  mutate(locus = if_else(locus %in% c("sll1773", "sml0004", "slr1609", "slr1939"), locus, "other")) %>%
...
ggarrange(ncol = 2, labels = c("A"), font.label = list_fontpars,
  plot_ncRNA_overview,
  ggarrange(nrow = 2, heights = c(0.65, 0.35), labels = c("B", "C"), font.label = list_fontpars,
    plot_asRNA_heat + theme(plot.margin = unit(c(26,12,-16,4), "points")),
    plot_asRNA_xy + theme(plot.margin = unit(c(4,12,12,12), "points")))
)
```

```{r, include = FALSE, eval = TRUE}
svg("../figures/plot_ncRNA_suppl.svg", width = 5.5, height = 7.5)
ggarrange(ncol = 2, labels = c("A"), font.label = list_fontpars,
  plot_ncRNA_overview,
  ggarrange(nrow = 2, heights = c(0.65, 0.35), labels = c("B", "C"), font.label = list_fontpars,
    plot_asRNA_heat + theme(plot.margin = unit(c(26,12,-16,4), "points")),
...
plot_ncRNA_upstream <- df_ncRNA_comp %>%
  ...
  scale_color_manual(values = custom_colors[c(1:3,5)])
...
plot_ncRNA_single <- df_main %>% filter(sgRNA_target %in% paste0("nc_", list_ncRNA_hits)) %>%
  ggplot(aes(x = time, y = log2FoldChange, color = factor(sgRNA_index))) +
  geom_point(size = 2) + ylim(-5, 5) +
  geom_smooth() +
  facet_grid( ~ sgRNA_target) +
  custom_theme() +
  scale_color_manual(values = custom_range(5))
```
...
## Analysis of ncRNA structure and function

Export list of interesting ncRNAs (subfigure D) to table or fasta file for further analysis with python.
