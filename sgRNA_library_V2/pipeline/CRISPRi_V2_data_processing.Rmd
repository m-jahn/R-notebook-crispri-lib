...
Since statistical significacne is tested for many genes in parallel, the p-value obtained from MWU test should be multiple-hypothesis corrected. For this purpose we use the Benjamini-Hochberg method. We also calculate a score taking both effect size and p-value into account, according to the publication from [Wang et al., Nat Comm, 2018](http://dx.doi.org/10.1038/s41467-018-04899-x). This score is simply the absolute fitness score multiplied by the negative log10 p-value.
...
plot_all_fitness_volc <- df_gene %>% filter(time == 0,
  ...
  arrange(sgRNA_type) %>%
  ggplot(aes(x = wmean_fitness, y = -log10(p_value_adj), shape = score >= 4, col = sgRNA_type)) +
  ...
  coord_cartesian(xlim = c(-6, 6), ylim = c(0, 4)) +
...
ggarrange(ncol = 2, widths = c(0.65, 0.35),
  ggarrange(nrow = 2, heights =  c(0.34, 0.66), labels = LETTERS[1:2], font.label = list_fontpars,
    plot_all_fitness_volc + theme(plot.margin = unit(c(12,-12,12,30), "points")),
