...
plot_tsne_linreg <- df_linreg_wide %>%
  inner_join(df_tsne, by = "locus") %>%
  filter(treatment != "intercept") %>%
  arrange(treatment, abs(slope)) %>%
  mutate(sgRNA_target = if_else(abs(slope) >= 2 | p_value <= 0.1, sgRNA_target, "")) %>%
  ggplot(aes(x = V1, y = V2, size = abs(slope),
    color = cut(p_value, breaks = c(0, 0.01, 0.05, 0.1, 1)), label = sgRNA_target)) +
...
```{r}
df_linreg_filtered <- df_linreg_wide %>% 
  filter(treatment != "intercept") %>% 
  group_by(locus) %>%
  filter(any(abs(slope) >= 2 & p_value <= 0.1)) %>%
...
color_table <- function(df, variable) {
  ...
  filter(if_any(paste0("p_value_", variable), ~ . <= 0.1)) %>%
...
df_unknown_hits <- df_linreg_filtered %>%
  ...
  # filter by effect: only slope > 3
  filter(if_any(matches("^(carb|light|\\-|\\+)"), ~ abs(.) > 3)) %>%
  select(-sgRNA_target, -gene_name, -gene_name_short, -protein,
    -length, -mass, -ec_number)
...
This list of genes (length: `r nrow(df_unknown_hits)`) is extracted and then simply fitness per condition is plotted as a line plot, in order to inspect the trends from fitting the multiple liner regression models. Note that this list of genes was derived from multiple linear regression. The obtained p-value used for filtering is differnet from the fitness p-value for the single conditions.
