...
for (cond in c("mixotroph", "photoheterotroph")) {
  ...
  SVG_mix <- set_attributes(SVG_template,
    node = filter(df_nakajima_mfa, condition == cond)$reaction_name,
    attr = "style",
  ...
  SVG_mix <- set_attributes(SVG_mix,
    node = filter(df_nakajima_mfa, condition == cond, flux < 0)$reaction_name,
    attr = "style",
...
plot_protmass_phycobilisome1 <- Jahn_2018_Light_and_CO2_lim %>%
  ...
  fill = protein)) +
  lims(y = c(0, 22)) +
  geom_col(position = "stack", width = 0.6, col = grey(1), size = 0.2) +
