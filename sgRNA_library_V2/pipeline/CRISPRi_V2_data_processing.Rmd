...
df_alignment <- read_lines("../data/output/ncRNA_alignment.txt") %>%
  ...
  filter(str_detect(genome, "2051288975")) %>%
...
```{r, fig.width = 8, fig.height = 7.5}
ggarrange(ncol = 3, labels = c("A"), font.label = list_fontpars,
  plot_ncRNA_overview,
  ggarrange(nrow = 2, heights = c(0.65, 0.35), labels = c("B", "C"), font.label = list_fontpars,
    plot_asRNA_heat + theme(plot.margin = unit(c(12,12,12,0), "points")),
    plot_asRNA_xy + theme(plot.margin = unit(c(4,12,16,12), "points"))),
  ggarrange(nrow = 2, heights = c(0.75, 0.25), labels = c("D", "E"), font.label = list_fontpars,
    plot_ncRNA_heat, ggplot() + custom_theme())
)
```
