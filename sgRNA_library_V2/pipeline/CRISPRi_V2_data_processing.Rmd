...
df_ncRNA <- df_main %>% filter(sgRNA_type == "ncRNA") %>%
  ...
  mutate(sgRNA_target = str_sub(sgRNA_target, 4, 100)) %>%
...
df_ncRNA %>% group_by(ncRNA_type) %>%
  summarize(n_targets = length(unique(sgRNA_target)))
```
...
```{r, fig.width = 4, fig.height = 4}
df_ncRNA %>% filter(time == 0, condition == "HC, HL") %>%
  ggplot(aes(x = wmean_fitness, y = -log10(p_value_adj), color = ncRNA_type)) +#score >= 4
...
df_ncRNA %>% filter(time == 0, ncRNA_type != "iTSS") %>%
  ggplot(aes(x = wmean_fitness, y = -log10(p_value_adj), color = ncRNA_type)) +#score >= 4
...
df_ncRNA %>% mutate(sgRNA_target = factor(sgRNA_target, unique(sgRNA_target)[df_ncRNA_order])) %>%
  mutate(wmean_fitness = wmean_fitness %>% replace(., . > 4, 4) %>% replace(., . < -4, -4)) %>%
  ...
  scale_fill_gradientn(colours = c(custom_colors[1], grey(0.9), custom_colors[2]),
    limits = c(-4, 4)) +
  facet_grid(ncRNA_type ~ ., scales="free")
```
