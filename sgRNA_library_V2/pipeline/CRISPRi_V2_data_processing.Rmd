...
df_linreg_wide %>%
  ...
  filter(if_any(matches("^pval_"), ~ abs(.) <= 0.05 )) %>%
  arrange(desc(r_squared)) %>%
  #arrange(across(matches("^pval_"), ~ sum(abs(.)))) %>%
  mutate(across(3:8, ~ cell_spec(., "html", color = "white",
      background = spec_color(., option = "E", scale = c(-5.5, 5.5)),
      bold = TRUE))) %>%
  kbl(format = "html", escape = F) %>%
...
```{r, fig.width = 8.0, fig.height = 3.5}
library(ggheatmap)

plot_sgRNAs_light <- df_linreg_wide %>%
  
  # make df with list of strongest correlated genes
  filter(if_any(matches("^(carb|light|\\-|\\+)"), ~ abs(.) >= 3 )) %>%
  filter(if_any(matches("^pval_"), ~ abs(.) <= 0.05 ))  %>% select(locus) %>%
  inner_join(df_gene, by = "locus") %>%
...
make_fitness_plot <- function(data, vars, title = NULL) {
  data %>% ungroup %>%
    ...
    group_by(sgRNA_target) %>% 
    mutate(significant = abs(wmean_fitness[1]-wmean_fitness[2])>4) %>%
    pivot_wider(names_from = condition, values_from = wmean_fitness) %>%
...
# this part is a bit problematic because many combinations appear twice,
# only in reverse order. 
# TODO: Need to fix that later
list_condition_pairs <- lapply(
  unique(df_gene$condition) %>% expand_grid(x = ., y = .) %>%
    filter(x != y) %>% t %>% as.data.frame %>% as.list,
  function(var) {
