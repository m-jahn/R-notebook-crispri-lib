...
plot_cluster_dend <- mat_cluster_sig %>%
  as.dendrogram %>% #reorder(., order(cutree(., k = 5))) %>% cutree(k = 5) %>%
  set("branches_k_col", custom_colors[c(3,5,1,4,2)], k = 5) %>%
...
- genes depleted **only in low CO2, low light** (air, 60 µE)
- genes enriched **only in high CO2, high light** (1% CO2, 1000 µE)

```{r}
df_gene <- mat_cluster_sig %>%
  as.dendrogram %>%
  cutree(k = 5) %>% enframe("locus", "cluster") %>%
...
- Genes strongly enriched **during high light exposure** (1% CO2, 1000 µmol photons m^-2 s^-1)
...
df_gene %>% filter(cluster == 5, time == 0) %>%
  ...
  geom_col(position = "dodge") +
  geom_errorbar(position = "dodge", size = 1) +
  custom_theme() + labs(title = "Cluster 5: enriched in high light, high CO2") +
...
df_gene %>% filter(str_detect(Gene.names, "[ac]pc"), time == 0) %>%
  ...
  labs(title = "Apc/Cpc mutants are also enriched in high light, high CO2") +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +
  scale_fill_manual(values = custom_colors) +
  scale_color_manual(values = custom_colors)
```
...
graph_cluster <- retrieve_STRING(
  gene_ID = filter(df_gene, cluster == 1) %>% pull(locus) %>% unique,
  taxon_ID = 1148,
  ref = df_gene %>% select(sgRNA_target, locus, Process, Pathway, Protein, Gene.names) %>% 
    distinct %>% rename(name = sgRNA_target)
)

graph_cluster %>% arrange(Process) %>% activate(edges) %>%
  filter(score >= 0.4) %>%
  ggraph(layout = 'linear', circular = TRUE) +
  geom_edge_arc(colour = grey(0.6, 0.5), aes(width = score)) + 
  geom_node_point(aes(colour = Process), size = 5) +
  geom_node_text(nudge_x = nudge_circle(74)$x, nudge_y = nudge_circle(74)$y, 
    size = 3.2, aes(label = name, colour = Process)) +
