...
```
Third, the target strand (template or coding) could be correlated to sgRNA efficiency. To analyze this in detail, we need to import a table with a subset of ~6000 sgRNAs that target genes where an effect was visible. For this set, we compare if the strand that an sgRNA is targeting is significantly correlated with sgRNA efficiency. As a result, there seems to be no significant effect.

```{r, fig.width = 4.5, fig.height = 3}
df_features_ml <- read_csv("../data/output/table_ml_features.csv", show_col_types = FALSE)
df_features_ml %>%
  mutate(strand = ifelse(strand == 0, "coding\nstrand", "template\nstrand")) %>%
  ggplot(aes(x = factor(strand), y = sgRNA_efficiency)) +
  facet_wrap(~ efficiency_binned, ncol = 2) +
  geom_violin(color = NA, fill = grey(0.9)) +
  geom_boxplot(notch = FALSE, outlier.shape = ".", fill = NA) +
  labs(x = "", y = "repression efficiency") +
  coord_cartesian(ylim = c(-0.15, 1.15)) +
  stat_summary(fun.data = function(x) c(y = median(x)+0.07,
    label = round(median(x), 2)), geom = "text", size = 3) +
  stat_summary(fun.data = function(x) c(y = 1.1,
...
Figure with log2 FC over generations for 2 examples, 5 random control guides and 5 guides targeting an essential ribosomal gene.
...
```{r, echo = FALSE}
save_plot(pl = plot_tsne_linreg, path = "../figures/", width = 8, height = 10)
```
...
```{r, include = FALSE}
svg(filename = "../figures/figure3.svg", width = 4.0, height = 7.0)
ggarrange(nrow = 4, heights =  c(0.33, 0.224, 0.224, 0.224),
  labels = LETTERS[1:4], font.label = list_fontpars,
  plot_centralcarb_minifig[[1]] + theme(plot.margin = unit(c(0, 0,-5,0), "points")),
  plot_centralcarb_minifig[[2]] + theme(plot.margin = unit(c(0 ,0,-5,0), "points")),
...
```{r, fig.width = 6.0, fig.height = 10.5}
...
```{r, include = FALSE}
svg(filename = "../figures/figure2.svg", width = 6.0, height = 10.5)
ggarrange(nrow = 4, heights =  c(0.32, 0.32, 0.13, 0.23),
  labels = LETTERS[1:4],
  font.label = list_fontpars,
  plot_photosystem2 + theme(plot.margin = unit(c(0, 12, 0, 12),"points")),
...
```{r, include = FALSE}
svg(filename = "../figures/figureS3_carbon_transport.svg", width = 6.0, height = 2.75)
...
Alternative representation of photosystems gene fitness as heat maps:
...
plot_sgRNAs_ps1 <- df_gene %>%
  ...
  geom_text(size = 4, nudge_y = -0.2, color = grey(0.4),
...
ggarrange(ncol = 2, labels = c("A", "B"), font.label = list_fontpars,
  plot_sgRNAs_ps1 + theme(plot.margin = unit(c(12, 12, 120, 12),"points")),
  plot_sgRNAs_ps2
)
```

```{r, include = FALSE, eval = TRUE}
svg("../figures/figureSX_photosystems.svg", width = 7.0, height = 5.2)
ggarrange(ncol = 2, labels = c("A", "B"), font.label = list_fontpars,
  plot_sgRNAs_ps1 + theme(plot.margin = unit(c(12, 12, 120, 12),"points")),
  plot_sgRNAs_ps2
)
...
```{r, fig.width = 6.5, fig.height = 6.5, warning = FALSE}
ggarrange(nrow = 2, labels = c("A"), font.label = list_fontpars, heights = c(0.52, 0.48),
  plot_ncRNA_overview + theme(plot.margin = unit(c(12, 12, -12, 12), "points")),
  ggarrange(ncol = 2, labels = c("B", "C"), font.label = list_fontpars,
    plot_ncRNAs_xy[[1]] + theme(plot.margin = unit(c(12, 12, 6, 12), "points")),
    plot_ncRNAs_xy[[2]] + theme(plot.margin = unit(c(12, 12, 6, 12), "points"))
  )
)
```

```{r, include = FALSE, eval = TRUE}
svg("../figures/figureSX_asRNA_iTSS.svg", width = 6.5, height = 6.5)
ggarrange(nrow = 2, labels = c("A"), font.label = list_fontpars, heights = c(0.52, 0.48),
  plot_ncRNA_overview + theme(plot.margin = unit(c(12, 12, -12, 12), "points")),
  ggarrange(ncol = 2, labels = c("B", "C"), font.label = list_fontpars,
    plot_ncRNAs_xy[[1]] + theme(plot.margin = unit(c(12, 12, 6, 12), "points")),
...
ggarrange(ncol = 2, labels = c("A", "B"), font.label = list_fontpars,
  ...
  ggarrange(nrow = 3, heights = c(0.43, 0.43, 0.14),
    ...
    plot_ncRNA_upstream[[1]],
    plot_ncRNA_upstream[[2]]
  )
)
```

```{r, include = FALSE, eval = TRUE}
svg("../figures/figure4.svg", width = 5.5, height = 6.5)
ggarrange(ncol = 2, labels = c("A", "B"), font.label = list_fontpars,
  plot_ncRNA_heat + theme(plot.margin = unit(c(12, 12, -20, 12), "points")),
  ggarrange(nrow = 3, heights = c(0.43, 0.43, 0.14),
    labels = c("", "C", ""), font.label = list_fontpars,
...
df_ncRNA_comp %>%
  select(sgRNA_target, condition, wmean_fitness, sd_fitness,
    p_fitness_adj, length, direction, upstream, downstream,
    upstream_gene_fitness, downstream_gene_fitness) %>%
  write_csv("../data/output/fitness_ncRNA_SI.csv")

lapply(c("upstream_gene_fitness", "downstream_gene_fitness"), FUN = function(var){
  lm(get(var) ~ wmean_fitness, df_ncRNA_comp) %>%
    summary
})
```
...
df_mu %>%
  mutate(statistical_test = "t-test, 2-sided, unpaired") %>%
  write.csv("../data/output/growth_selected_mutants.csv")
```
...
# save summary table as supplemental file
df_gene %>%
  filter(sgRNA_type == "gene") %>%
  select(gene_name, locus, condition, wmean_fitness, p_fitness_adj) %>%
  distinct %>%
  group_by(locus) %>%
  filter(any(p_fitness_adj <= 0.01)) %>%
  rename(fitness = wmean_fitness, padj = p_fitness_adj) %>%
  pivot_wider(names_from = condition, values_from = c("fitness", "padj")) %>%
  arrange(locus) %>%
  write_csv("../data/output/Supplemental_Data_Set_2.csv")
```
