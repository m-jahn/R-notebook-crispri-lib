...
As a Supplementary Figure, we can plot the **total protein mass of the phycobilisome** determined by protein mass spectrometry.
...
plot_protmass_phycobilisome1 <- Jahn_2018_Light_and_CO2_lim %>%
  ...
  fill = protein)) +
  lims(y = c(0, 22)) +
  geom_col(position = "stack", width = 0.6, col = grey(0.8)) +
  ...
  scale_fill_manual(values = custom_range(13)) +
  scale_color_manual(values = custom_range(13))
...
ggarrange(ncol = 2, widths = c(0.5,0.5),
  ...
  plot_protmass_phycobilisome2 +  theme(plot.margin = unit(c(12,12,78,12), "points"))
)
```

```{r, include = FALSE}
svg("../figures/plot_protmass_phycobilisome.svg", width = 6.5, height = 4)
ggarrange(ncol = 2, widths = c(0.5,0.5),
  labels = LETTERS[1:2], font.label = list_fontpars,
  plot_protmass_phycobilisome1,
  plot_protmass_phycobilisome2 +  theme(plot.margin = unit(c(12,12,78,12), "points"))
