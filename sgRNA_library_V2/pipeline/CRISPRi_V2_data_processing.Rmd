...
```{r, fig.width = 5.5, fig.height = 3.3}
plot_STRING(
...
```{r, fig.width = 6.8, fig.height = 3.3}
plot_STRING(
...
```{r}
# select mean fitness and one condition only
...
df_gene %>% ungroup %>%
  ...
  select(locus, sgRNA_target, condition, wmean_fitness, Process, Pathway, Protein) %>% 
  distinct %>%
  pivot_wider(names_from = condition, values_from = wmean_fitness) %>%
