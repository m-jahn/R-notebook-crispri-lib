...
df_ncRNA %>% filter(time == 0) %>%
  ...
  select(condition, sgRNA_target, ncRNA_type, score) %>%
  summarize(n_targets = length(unique(sgRNA_target)), sig = sum(score >= 4),
    non_sig = sum(score < 4)) %>%
  summarize(n_targets = n_targets[1], `significant (score >= 4)` = mean(sig),
...
svg("../figures/plot_ncRNA_suppl.svg", width = 5.5, height = 5.5)
ggarrange(nrow = 2, labels = c("A"), font.label = list_fontpars,
  plot_ncRNA_overview,
  ggarrange(ncol = 2, labels = c("B", "C"), font.label = list_fontpars,
    plot_asRNA_xy, plot_iTSS_xy)
...
Export list of interesting ncRNAs (subfigure B/C) to table or fasta file for further analysis with python.
