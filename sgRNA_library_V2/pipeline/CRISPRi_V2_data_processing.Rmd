...
plot_heatmap_all <- levelplot(mat_heatmap,
  ...
  col.regions = colorRampPalette(c(custom_colors[1], grey(0.9), custom_colors[2]))(20),
...
We filter all ncRNAs that have a "significance" equivalent to a fitness score abs(F) >= 2 and -log10 p-value >= 2 (alpha = 0.01).
Significance here means effect size (F) multiplied by p-value.

```{r, fig.width = 6.5, fig.height = 5}
df_gene %>% filter(sgRNA_type == "ncRNA", time == 0) %>%
  ggplot(aes(x = wmean_fitness, y = -log10(p_value_adj), color = score >= 4)) +
...
df_ncRNAs <- filter(df_main, sgRNA_type == "ncRNA") %>%
  filter(length(unique(sgRNA_position)) >= 2) %>%
  select(sgRNA_target) %>% distinct %>%
  inner_join(df_gene, by = "sgRNA_target") %>%
  filter(time == 0) %>%
  filter(any(score >= 4))

# cluster sgRNAs by fitness and obtain order
df_ncRNAs_order <- df_ncRNAs %>%
  select(sgRNA_target, condition, wmean_fitness) %>%
  pivot_wider(names_from = condition, values_from = wmean_fitness) %>%
  column_to_rownames(var = "sgRNA_target") %>%
  as.matrix %>% dist %>% hclust(method = "ward.D2") %>%
  as.dendrogram %>% order.dendrogram()
  
df_ncRNAs %>% ungroup %>%
  mutate(sgRNA_target = factor(sgRNA_target, unique(sgRNA_target)[df_ncRNAs_order])) %>%
  mutate(wmean_fitness = wmean_fitness %>% replace(., . > 4, 4) %>% replace(., . < -4, -4)) %>%
  group_by(sgRNA_target) %>% filter(mean(wmean_fitness) > -0.5) %>%
  ggplot(aes(x = condition, y = sgRNA_target, fill = wmean_fitness)) +
  geom_tile() + custom_theme(legend.pos = "bottom") +
  labs(title = "ncRNA fitness", x = "", y = "") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) +
  scale_fill_gradientn(colours = c(custom_colors[1], grey(0.9), custom_colors[2]),
    limits = c(-4, 4))
...
df_ncRNAs %>% ungroup %>%
  mutate(sgRNA_target = factor(sgRNA_target, unique(sgRNA_target)[df_ncRNAs_order])) %>%
  mutate(wmean_fitness = wmean_fitness %>% replace(., . > 4, 4) %>% replace(., . < -4, -4)) %>%
  group_by(sgRNA_target) %>% filter(mean(wmean_fitness) < -2) %>%
  ggplot(aes(x = condition, y = sgRNA_target, fill = wmean_fitness)) +
  geom_tile() + custom_theme(legend.pos = "bottom") +
  labs(title = "ncRNA fitness", x = "", y = "") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) +
