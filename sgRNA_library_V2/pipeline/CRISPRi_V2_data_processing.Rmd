...
df_gene %>% 
  filter(cluster == 4, time == 0) %>%
  #filter(grepl("gap", Gene.names), time == 8) %>%
  arrange(cluster_order) %>% 
  mutate(sgRNA_target = factor(sgRNA_target, unique(sgRNA_target))) %>%
  mutate(wmean_fitness = wmean_fitness %>% replace(., .>3, 3) %>% replace(., .<-3, -3)) %>%
  ggplot(aes(x = sgRNA_target, y = condition, fill = wmean_fitness)) +
...
To plot gene fitness for the enzymes of central carbon metabolism, we need a complete list of enzymes and the genes that they are mapped to. We can list the different KEGG databases that can be queried:

```{r}
listDatabases()
```

Next, gene-pathway mappings are obtained and merged with pathway names and gene/enzyme names.
...
df_kegg <- keggLink("pathway", "syn") %>%
  ...
  mutate(index = rep(1:10, length.out = n())) %>%
  group_by(index) %>%
  mutate(kegg_gene = keggList(locus)) %>%
