...
```{r}
# if getting a current table from uniprot fails due to faulty connection,
# obtain a local fallback copy
uniprot_error <- simpleError("Uniprot server not available")
df_uniprot <- tryCatch(
  read_tsv(paste0(
...
save_plot(plot_sgRNA_ribo_example, width = 7, height = 4.5)
```
...
```{r, warning = FALSE}
determine_corr <- function(index, value, group) {
...
df_main <- df_main %>%
  group_by(sgRNA_target) %>%
  mutate(sgRNA_correlation = determine_corr(sgRNA_index, log2FoldChange, group))
```
...
The following two plots show single sgRNAs and fitness for a tricky case, `gltA`. The strongest depletion is achieved by sgRNA 1 but in mean fitness quantification, it doesn't show. The reason is ....
...
df_gene %>% filter(sgRNA_target == "gltA") %>%
  pivot_longer(cols = matches("[n12]_log2FoldChange"), names_to = "metric", values_to = "log2FoldChange") %>%
  mutate(metric = str_remove(metric, "_log2FoldChange")) %>%
  ggplot(aes(x = time, y = log2FoldChange, 
    ymin = log2FoldChange-sd_log2FoldChange, 
...
df_main %>% filter(grepl("gltA", sgRNA_target)) %>% ungroup %>%
  select(sgRNA_position, sgRNA_correlation, sgRNA_efficiency) %>% distinct %>%
  mutate(sgRNA_weight = sgRNA_correlation*sgRNA_efficiency) %>%
  arrange(sgRNA_position)
```
...
save_plot(plot_all_fitness_hist, width = 7, height = 4)
```
...
save_plot(plot_controls_sgRNAs, width = 7, height = 4)
```
...
save_plot(plot_heatmap_all, width = 8, height = 1.6)
```
...
```{r, include = FALSE}
png("../figures/plot_heatmap_signif.png", width = 1600, height = 600, res = 120)
gridExtra::grid.arrange(
  # coords for unit: top, right, bottom, left
  plot_cluster_dend + 
    theme(plot.margin = unit(c(0.1, 0.03, -0.2, 0.146),"npc")),
  plot_heatmap_sig,
  nrow = 2
)
invisible(capture.output(dev.off()))
...
ggsave("../figures/plot_nmds_tsne.svg", plot = gridExtra::arrangeGrob(plot_nmds, plot_tsne, ncol = 2),
...
plot_sgRNAs_cl1 <- df_gene %>% filter(cluster == 1, time == 0) %>%
...
print(plot_sgRNAs_cl1)
save_plot(plot_sgRNAs_cl1, width = 7.5, height = 3.5)
```

More space-efficient representation as a heat map.

```{r, fig.width = 5.0, fig.height = 2.5}
plot_sgRNAs_cl1_hm <- df_gene %>% filter(cluster == 1, time == 0) %>%
...
print(plot_sgRNAs_cl1_hm)
save_plot(plot_sgRNAs_cl1_hm, width = 5.0, height = 2.5)
```
...
print(plot_sgRNAs_cl5_hm)
save_plot(plot_sgRNAs_cl5_hm, width = 10.5, height = 2.5)
```

**Cluster 4**: Genes depleted **on low light** (0.05 or 1% CO2, 60 µmol photons m^-2 s^-1)

```{r, fig.width = 8.5, fig.height = 2.5}
plot_sgRNAs_cl4_hm <- df_gene %>% filter(cluster == 4, time == 0) %>%
  arrange(cluster_order) %>% 
  mutate(sgRNA_target = factor(sgRNA_target, unique(sgRNA_target))) %>%
  mutate(wmean_fitness = wmean_fitness %>% replace(., . > 3, 3) %>% replace(., . < -3, -3)) %>%
  ggplot(aes(x = sgRNA_target, y = condition, fill = wmean_fitness)) +
...
print(plot_sgRNAs_cl4_hm)
save_plot(plot_sgRNAs_cl4_hm, width = 8.5, height = 2.5)
```
...
plot_sgRNAs_phycobil <- df_gene %>%
  ...
  mutate(wmean_fitness = wmean_fitness %>% replace(., . > 2, 2) %>% replace(., . < -2, -2)) %>%
  ggplot(aes(x = sgRNA_target, y = condition, fill = wmean_fitness)) +
  geom_tile() + custom_theme() +
...
save_plot(plot_sgRNAs_phycobil, width = 6.5, height = 2.5)
```
...
plot_one_on_one <- gridExtra::arrangeGrob(nrow = 3, ncol = 2,
...
ggsave("../figures/plot_one_on_one.svg", plot_one_on_one, 
  width = 8, height = 12)
```
...
ggsave("../figures/plot_fitness_glycolysis.svg", plot_gene_fitness[[1]],
  width = 8, height = 6)
```

```{r, fig.width = 8, fig.height = 5}
print(plot_gene_fitness[[2]])
ggsave("../figures/plot_fitness_pentose.svg", plot_gene_fitness[[2]],
  width = 8, height = 5)
```

```{r, fig.width = 8, fig.height = 5}
print(plot_gene_fitness[[3]])
ggsave("../figures/plot_fitness_carbonfix.svg", plot_gene_fitness[[3]],
  width = 8, height = 5)
```

```{r, fig.width = 8, fig.height = 11}
print(plot_gene_fitness[[4]])
ggsave("../figures/plot_fitness_photosys.svg", plot_gene_fitness[[4]],
  width = 8, height = 11)
```

```{r, fig.width = 8, fig.height = 4}
print(plot_gene_fitness[[5]])
ggsave("../figures/plot_fitness_antenna.svg", plot_gene_fitness[[5]],
  width = 8, height = 4)
```

```{r, fig.width = 8, fig.height = 4}
print(plot_gene_fitness[[6]])
ggsave("../figures/plot_fitness_citrate.svg", plot_gene_fitness[[6]],
  width = 8, height = 4)
```
...
ggsave("../figures/plot_fitness_stress.svg", plot_stress_genes,
  width = 8, height = 4)
```
...
```{r, echo = FALsE}
plot_carboxysome <- df_gene %>%
  filter(time == 0, locus %in% c(
  ...
  ggplot(aes(x = condition, y = wmean_fitness, 
    ...
    custom_theme(aspect.ratio = 1,
      legend.position = "bottom", legend.key.size = unit(0.4, "cm")) + 
    labs(title = "Carboxysome and carbon transporters") +
    theme(axis.text.x = element_blank()) +
    scale_fill_manual(values = custom_colors) +
    scale_color_manual(values = custom_colors) +
    facet_wrap(~ sgRNA_target, ncol = 8)
...
```{r, fig.width = 8, fig.height = 4}
print(plot_carboxysome)
ggsave("../figures/plot_fitness_carboxysome.svg", plot_carboxysome,
  width = 8, height = 4)
```
...
df_main %>% filter(sgRNA_type == "gene") %>%
  write_csv("../data/output/fitness_all.csv")
