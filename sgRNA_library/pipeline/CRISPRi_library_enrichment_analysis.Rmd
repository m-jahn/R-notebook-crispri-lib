...
df <- ungroup(df) %>% mutate(condition = factor(condition, c("L100", "L300", "LD", "Lactate", "NaCl")))
...
paste(df$sgRNA %>% unique %>% length, "unique sgRNAs targeting ORFS")
paste(df_ncRNA$sgRNA %>% unique %>% length, "unique sgRNAs targeting ncRNAs")
# Number of ORFs targeted with 1 or 2 sgRNAs
df %>% filter(!duplicated(sgRNA)) %>% group_by(sgRNA_short) %>% 
  summarize(num_sgRNAs = length(sgRNA_index)) %>% pull(num_sgRNAs) %>% table
# Number of ncRNAs targeted with 1 or 2 sgRNAs
df_ncRNA %>% filter(!duplicated(sgRNA)) %>% group_by(sgRNA_short) %>% 
  summarize(num_sgRNAs = length(sgRNA_index)) %>% pull(num_sgRNAs) %>% table

# Missing sgRNA quantifications per condition, higher for Lactate
...
Out of the 39 possible conditions, not every sgRNA was quantified in all conditions.
Here is a summary for quantification of 7119 ORF-targeting and 3379 ncRNA targeting 
genes.

```{r, fig.height = 3, fig.width = 6}
bind_rows(.id = "sgRNA_target",
    df %>% group_by(sgRNA) %>% summarize(quantified = sum(!is.na(FC))),
    df_ncRNA %>% group_by(sgRNA) %>% summarize(quantified = sum(!is.na(FC)))
  ) %>% 
  
  mutate(sgRNA_target = recode(sgRNA_target, `1` = "ORFs", `2` = "ncRNAs")) %>%
  group_by(sgRNA_target) %>%
  arrange(quantified) %>%
  mutate(index = 1:length(quantified)) %>%
  #filter(sgRNA_target == "ORFs") %>% pull(quantified) %>% table
  
  xyplot(quantified ~ index | sgRNA_target, .,
    as.table = TRUE, scales = list(alternating = FALSE),
    xlab = "sgRNA", ylab = "quantified in n samples",
    par.settings = custom.lattice, 
    between = list(x = 0.5, y = 0.5),
    type = "b", pch = 19, cex = 0.6, lwd = 2.5,
    panel = function(x, y, ...) {
...
Abbreviations: L100 - light with 100 µmol photons m<sup>-2</sup> s<sup>-1</sup>, L300 - light with 300 µmol photons m<sup>-2</sup> s<sup>-1</sup>, LD - light-dark-cycle, Lactate - addition of L-lactate, NaCl - addition of sodium chloride.
...
plot_2sgRNA_correlation <- xyplot(`sgRNA-2` ~ `sgRNA-1` | 
  ...
  xlab = bquote(log[2] ~ "fold change sgRNA1"), ylab = bquote(log[2] ~ "fold change sgRNA2"),
  panel = function(x, y, ...) {
    panel.grid(h = -1, v = -1, col = grey(0.9))
    panel.xyplot(x, y, alpha = 0.3, ...)
    if (panel.number() %in% c(1:5, 7:11, 17:18, 23:24, 29:30)) {
      panel.quadrants(x, y, h = -2, v = -2, lwd = 2, col = grey(0.3), na.rm = TRUE)
    }
    panel.key(which.panel = 6, c("induced", "uninduced"), pch = 19, 
      col = c("#E6AB02", "#7570B3"), corner = c(0.1, 0.9))
  }
)
...
L300 - light with 100 µmol photons m<sup>-2</sup> s<sup>-1</sup>, 
LD - light-dark cycle, Lactate - addition of L-lactate, 
NaCl - addition of sodium chloride.
...
is  in 'long' format (one variable per column, one observation per row). 
...
mat <- unite(df, condition, condition, induction, timepoint) %>%
  ...
  # reorder columns and exclude Lactate/NaCl data
...
number of 2 to 8 offers good separation, while higher cluster numbers reduce
...
clusters <- clusters %>% recode(`5` = 4L, `4` = 5L, `6` = 5L) %>% 
...
# custom color vector for clusters
...
# display colors as a test
plot(x = 1:5, y = rep(1, 5), pch = 19, cex = 4, col = custom_palette)
```
...
plot_heatmap <- levelplot(
  ...
  col.regions = colorRampPalette(rev(RColorBrewer::brewer.pal(5, "PuOr"))),
...
plot_library_depletion <- xyplot(log2FoldChange ~ generations | paste(condition, " - ", cluster),
  ...
  xlab = "generations", ylab = bquote(log[2] ~ "fold change"), 
...
As a supplemental information, we tested also differential fitness
between L100 and LD (25 genes with both sgRNAs present), and L300 and LD (4 genes with 
both sgRNAs present). The overlap
  ...
  xyplot(fitness_score ~ factor(condition, light_conditions) | factor(cluster), .,
    ...
    panel = function(x, y, ...) {
      ...
      panel.stripplot(as.numeric(x)-0.25, y, jitter.data = TRUE, amount = 0.14, 
        pch = 19, cex = 0.3, horizontal = FALSE, alpha = 0.2, ...)
...
plot_diff_fitness <- xyplot(diff_fitness_score ~ factor(Process.abbr) | factor(cluster), 
  ...
  xlab = "", ylab = paste0("dF (",light_conditions[2], "-", light_conditions[1], ")"),
  par.settings = custom_lattice, type = "p", pch = 19, as.table = TRUE, 
  col = c("#E7298A", grey(0.6)),
...
plot_top_diff_fitness <- xyplot(log2FoldChange ~ generations | 
    ...
    par.settings = {cust = custom_lattice;
      cust$superpose.polygon$col = c("#E6AB02", "#E6AB02", "#7570B3", "#7570B3"); cust}, 
    type = "l", layout = c(6, 3), 
    scales = list(alternating = FALSE, cex = 0.8),
    ylim = c(-10, 10), xlim = c(0, 17), ylab = bquote(log[2] ~ "fold change"),
    panel = function(x, y, ...) {
      panel.grid(h = -1, v = -1, col = grey(0.9))
      panel.key(light_conditions, points = FALSE, col = c("#7570B3", "#E6AB02"),
...
```{r, results = 'hide', include = FALSE}
  # Supplemental Dataset S5: table with most different sgRNAs as *.csv
  filter(df, 
    sgRNA_short %in% (filter(df_selected_fitness, 
      !between(diff_fitness_score, -3, 3)) %>% pull(sgRNA_short)),
...
#### Venn diagram to explore overlap between selected gene sets

The following code chunk is not supposed to be executed regularly.
It is only an additional comparison requested during review of the submitted manuscript.
It requires that the character vector `df_selected_genes` is copied with a new name 
corresponding to each of the three light comparisons, see three vectors used below.

```{r, eval = FALSE}
# make a Venn diagram of the three gene vectors
venn.diagram(list(L100_L300 = L100_L300, L100_LD = L100_LD, L300_LD = L300_LD),
  imagetype = "svg", height = 4, width = 4, 
  filename = "../figures/supplemental/venn_light_conditions2.svg", 
  fontfamily = "FreeSans", cat.fontfamily = "FreeSans",
  fill = c("#E4E9F2", "#D9F0E5", "#FEE5D5"),
  col = "white"
)
  ```
...
between LD and L300, and they dont form a distinct cluster. The result from this 
...
filtered. The volcano plot shows enrichment of 75 sgRNAs under lactate after 32 d,
(induced condition, threshold: log2 FC $\geq$ 2, -log10 p-value $\geq$ 20), 
while only 8 sgRNAs were enriched in the NaCl control. 
Of these 75 sgRNAs, 16 were targeting the same gene
...
plot_lactate_volcano <- df %>% filter(condition %in%  c("Lactate", "NaCl") & timepoint > 8) %>% 
  ...
  xyplot(-log10(padj) ~ log2FoldChange | 
    ...
    col = c("#E6AB02", "#7570B3"),
    cex = 0.3, pch = 19, between = list(x = 0.5, y = 0.5),
    scales = list(alternating = FALSE),
    ylim = c(- 20, 200), xlim = c(-10, 10),
    panel = function(x, y, groups, subscripts, ...) {
      panel.grid(h = -1, v = -1, col = grey(0.9))
      panel.xyplot(x, y, groups = groups, subscripts = subscripts, alpha = 0.3, ...)
      # only quadrants for induced genes
      panel.quadrants(x[groups[subscripts] == "i"], y[groups[subscripts] == "i"], 
        h = 20, v = 2, labels = "events")
      panel.key(c("induced", "uninduced"), col = c("#E6AB02", "#7570B3"), 
...
# (75 sgRNAs in 32d induced, 22 uninduced. The 24 sgRNAs in 16d sample are all
# contained in 75 sgRNAs at 32d)
candidates <- filter(df, condition %in% "Lactate", 
  induction == "i",
...
candidates %in% falsepos %>% any
```


```{r}
# generate subset df with candidate sgRNAs only
df_lac <- filter(df, sgRNA %in% candidates & condition %in% c("NaCl", "Lactate"))
head(df_lac)
```
...
df_selected_fitness <- df_lac %>% 
  ...
  # consider only Lactate, induced condition
  group_by(sgRNA_short) %>% 
  filter(condition == "Lactate", induction == "i") %>%
...
gradient <- RColorBrewer::brewer.pal(8, "Dark2")[1:7]
custom_lattice$superpose.polygon$col <- gradient
custom_lattice$superpose.line$col <- gradient
...
plot_1sgRNAs <- xyplot(log2FoldChange ~ generations | 
  ...
  filter(condition == "Lactate", induction == "i", length(sgRNA_index) == 6), 
  auto.key = list(columns = 4, points = FALSE, lines = TRUE),
  as.table = TRUE, groups = Process.abbr, ylab = bquote(log[2] ~ "fold change"),
  par.settings = custom_lattice, type = "p",
  scales = list(alternating = FALSE, cex = 0.8),
...
```{r, results = 'hide', include = FALSE}
svg("../figures/supplemental/Supplemental_figure_lactate_enrichment_1sgRNA.svg", width = 8, height = 6)
print(plot_1sgRNAs)
dev.off()
```
...
```{r}
custom_palette <- RColorBrewer::brewer.pal(8, "Dark2")[c(4,5,6,3,1)]
custom_lattice$superpose.polygon$col <- custom_palette
custom_lattice$superpose.line$col <- custom_palette
...
plot_2sgRNAs <- xyplot(log2FoldChange ~ generations | 
  factor(sgRNA_short, arrange(df_lac, Process.abbr) %>% 
    pull(sgRNA_short) %>% unique),
  df_lac %>% group_by(condition, sgRNA_short) %>%
    filter(condition == "Lactate", induction == "i", length(sgRNA_index) == 12), 
...
custom_palette <- RColorBrewer::brewer.pal(8, "Dark2")[c(8,4,5,6,3)]
custom_lattice <- custom.lattice()
custom_lattice$superpose.symbol$col <- custom_palette
custom_lattice$superpose.line$col <- custom_palette
...
plot_knockouts_OD <- bind_rows(
  ...
  # filter to include only NT0 control, bcp2, ilvA (1st replicate ) and aroH
  filter(
    between(batchtime_h, 0, 100), od_led == 720,
    (cultivation == 1 & channel_id %in% c(5,6)) | 
    (cultivation == 2 & channel_id %in% c(1,2,3,7,8))
  ) %>%
  
  mutate(strain = NA) %>%
  mutate_cond(cultivation == 1 & channel_id %in% c(5,6), strain = "aroH") %>%
  mutate_cond(cultivation == 2 & channel_id %in% c(1,2), strain = "NT0") %>%
  mutate_cond(cultivation == 2 & channel_id %in% c(3  ), strain = "ilvA") %>%
  mutate_cond(cultivation == 2 & channel_id %in% c(7,8), strain = "bcp2") %>%
  
  # correct measurements for 1 ilvA replicate where OD was distorted by ceiling (?)
  mutate_cond(strain == "ilvA" & between(batchtime_h, 90.5, 94.75) & od_led == 720,
    od_corr = od_corr-0.03) %>%
  
  # plot
  xyplot(od_corr ~ batchtime_h, .,
    groups = factor(strain, c("NT0", "bcp2", "aroH", "ilvA")), 
    ...
    panel = function(x, y, groups, subscripts, ...) {
      ...
      panel.key(points = FALSE, lines = TRUE, 
        lwd = 1.5, cex = 0.65, groups = groups)
    }
  )
...
plot_knockouts_mu <- read_csv("../raw_data/growth_rate_knockouts.csv") %>%
  ...
  xyplot(mean_growth_rate ~ factor(strain, unique(strain)[c(6,2,9,8,10,4,7,5,1,3)]), .,
    ...
    as.table = TRUE, par.settings = custom_lattice, col = grey(0.2),
    fill = c("white", rep(grey(0.9), 9)),
    ...
    panel = function(x, y, ...) {
      ...
      panel.pvalue(x, y, symbol = TRUE, cex_symbol = 1.2, alternative = "greater",
        col = grey(0.2), offset = 0.2, verbose = TRUE)
    }
  )
...
df_abs <- read_csv("../raw_data/abs_spectrum_lactate.csv") %>%
  ...
  filter(strain %in% c("NT0", "bcp2", "aroH", "ilvA")) %>%
  
  # reshape strain to factor
  mutate(strain = factor(strain, c("NT0", "bcp2", "aroH", "ilvA")))
...
plot_lac_spectra_1 <- xyplot(absorption ~ wavelength_nm, df_abs, groups = strain,
  ...
  ylim = c(1.0, 2.4), xlim = c(350, 750),
  xlab = "wavelength [nm]", ylab = "absorption [a.u.]",
  lwd = 1.5, scales = list(alternating = FALSE), as.table = TRUE,
  panel=function(x, y, ...) {
    panel.grid(h = -1, v = -1, col = grey(0.9))
    panel.xyplot(x, y, ...)
    ...
    panel.key(points = FALSE, cex = 0.7, corner = c(0.01, 0.05), ...)
  }
)
...
plot_lac_spectra_2 <- df_abs %>% 
  ...
  xyplot(absorption ~ strain | factor(pigment, unique(pigment)[3:1]), ., 
    groups = strain,
    par.settings = custom_lattice, lwd = 2, pch = 19,
...
print(plot_knockouts_mu, position = c(-0.015, 0.22, 0.63, 0.49), more = TRUE)
...
```{r, results = 'hide', include = FALSE}
svg("../figures/main/Figure5_barchart.svg", width = 6, height = 10)
print(plot_lactate_volcano, position = c(0, 0.76, 1, 1), more = TRUE)
print(plot_2sgRNAs, position = c(0.01, 0.45, 1, 0.79), more = TRUE)
print(plot_knockouts_mu, position = c(-0.015, 0.22, 0.63, 0.49), more = TRUE)
print(plot_knockouts_OD, position = c(0.6, 0.24, 1, 0.49), more = TRUE)
...
```{r}
# filter for sgRNAs different between Lactate and NaCl
candidates_depl <- filter(df, 
    condition %in% c("NaCl", "Lactate"), 
    induction == "i", timepoint == 0) %>%
  
  # optional filter for sgRNAs that have both sgRNAs detected in NaCl and Lactate
  group_by(sgRNA_short) %>% filter(length(sgRNA_index) == 4) %>%

  # filter by highest fitness diff between sgRNA of two conditions (NaCl-Lactate)
  # higher for stronger Lactate depletion
...
```{r}
# generate subset df with candidate sgRNAs only
df_lac_depl <- filter(df, sgRNA %in% candidates_depl, condition %in% c("NaCl", "Lactate"))
head(df_lac_depl)
```
...
```{r}
custom_palette <- RColorBrewer::brewer.pal(8, "Dark2")[1:7]
custom_lattice$superpose.symbol$col <- custom_palette
custom_lattice$superpose.line$col <- custom_palette
...
```{r, results = 'hide', include = FALSE}
svg("../figures/supplemental/Supplemental_figure_Lactate_depletion.svg", width = 8, height = 12)
print(plot_depl_1sgRNAs, position = c(0, 0.21, 1, 1), more = TRUE)
print(plot_depl_2sgRNAs, position = c(0, 0, 1, 0.22))
grid.text(label = c("a", "b"), x = c(0.03, 0.03), y = c(0.98, 0.22), gp = gpar(cex = 1.2))
dev.off()
