...
plot_knockouts_OD <- bind_rows(
  ...
  xyplot(od_corr ~ batchtime_h, .,
    group = factor(strain, unique(strain)[c(2,3,1)]), xlab = "time [h]", ylab = "OD 720nm",
...
plot_knockouts_mu <- read_csv("../raw_data/growth_rate_knockouts.csv") %>%
  ...
  xyplot(mean_growth_rate ~ factor(strain, unique(strain)[c(6,2,5,9,8,10,4,7,1,3)]), .,
    xlab = "strain", ylab = bquote('mean' ~ mu),
...
df_abs <- read_csv("../raw_data/abs_spectrum_lactate.csv") %>%
  ...
  filter(strain %in% c("NT0", "bcp2", "aroH", "sll1496", "gltX", "spoT")) %>%
  
  # reshape strain to factor
  mutate(strain = factor(strain, c("NT0", "bcp2", "aroH", "sll1496", "gltX", "spoT")))
...
print(plot_lac_spectra_2, position = c(0.4, -0.01, 1, 0.25))
grid.text(label = c("A", "B", "C", "D", "E", "F"), gp = gpar(cex = 1.2),
  x = c(0.03, 0.03, 0.03, 0.6, 0.03, 0.4), 
...
**C.** Mean growth rate of selected knock-down strains (n = 2) over the first 
**D.** To be added.
