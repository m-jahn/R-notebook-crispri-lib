...
custom_palette <- colorspace::qualitative_hcl(
  n = length(unique(clusters)), h = c(30, 360), c = 70, l = 50)
...
plot_heatmap <- levelplot(
  ...
  col.regions = colorRampPalette(c(custom_palette[4], grey(0.98), custom_palette[1])),
...
plot_lactate_volcano <- df %>% filter(condition %in%  c("Lactate", "NaCl") & timepoint > 8) %>% 
  ...
  xyplot(-log10(padj) ~ log2FoldChange | 
    ...
    panel = function(x, y, groups, subscripts, ...) {
      ...
      panel.key(c("induced", "uninduced"), col = c("#D33F6A", "#00989D"), 
