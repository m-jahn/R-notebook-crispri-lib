...
The correct calculation of read fraction is controlled by the following steps, 
...
combinations <- list(
  testCond = Samples$group %>% unique %>% 
    grep(pattern = "_(1|2|4|8|16|32|12|30)", value = TRUE),
  refCond = rep(Samples$group %>% unique %>% 
    grep(pattern = "_0", value = TRUE), c(2,2,5,5,2,2,5,5,2))
)
...
Quality control: check manual log2 FC versus DESeq2 log2 FC
...
plot_volcanoplot <- df %>%
  ...
  xyplot(-log10(padj) ~ log2FoldChange | factor(timepoint) * condition, ., 
    as.table = TRUE, groups = induction,
    par.settings = custom.lattice, 
    cex = 0.2, pch=19,
    scales = list(alternating = FALSE),
...
Write processed data frame to file and continue with analysis
in next script
