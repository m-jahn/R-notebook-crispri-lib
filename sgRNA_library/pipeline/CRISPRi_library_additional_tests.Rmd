...
plot_volcanoplot <- df_rnaseq %>%
  ...
  xyplot(-log10(padj) ~ log2FoldChange | Condition %>% factor(., unique(.)[c(2,3,4,5,1)]), ., 
...
genes between mutants. One way is to use a heat map.
...
plot_heatmap <- df_rnaseq %>% filter(Name %in% sig_genes) %>%
  
  # limit plotted range to log2 FC of -3 to 3 (8 fold)
  mutate(log2FoldChange = 
    log2FoldChange %>% replace(., . > 3.5, 3.5) %>% replace(., . < -3.5, -3.5)) %>%
  ...
  # reorder matrix by simple clustering
  column_to_rownames("Name") %>% as.matrix %>% replace_na(0) %>%
  {.[dist(.) %>% hclust(., method = "ward.D2") %>% {.$order}, ]} %>%
  
  # plot genes
  levelplot(
    col.regions = colorRampPalette(c("steelblue", grey(0.95), "darkorange"))(16),
...
observed increased growth rate.
...
# Alternatively we can try kegga pathway enrichment.

sig_genes %>% kegga(species.KEGG = "syn", FDR = 0.05, trend = FALSE) %>%
  arrange(P.DE) %>% filter(P.DE <= 0.1)
```

The only clear pattern is that genes involved in Amino acid metabolism are
enriched. But since the dataset is small, the enrichment is not very significant.
No pathway is really sticking out from the background. We established already that
there seems to be a large overlap in differentially expressed genes for all
mutants except `pmgA`. We can further elaborate this by plotting log2 FC for the 
subset of differentially expressed genes against each other for all mutants.
