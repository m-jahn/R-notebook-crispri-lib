...
grid.text(label = c("A", "B", "C", "D"), x = rep(0.03, 4), 
  y = c(0.98, 0.8, 0.56, 0.44), gp = gpar(cex = 1.3))
```
...
**A.** 
**B.** Volcanoplot of log<sub>2</sub> fold change (log<sub>2</sub> FC) against
...
-1 $\geq$ log<sub>2</sub> FC $\geq$ 1). The p-value for three sgRNA mutants was outside 
the plotting region and was restricted to -log10(p) = 50.
