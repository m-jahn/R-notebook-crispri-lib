...
df_lac_prod <- bind_rows(.id = "setup",
  ...
  # fitler strains and adjust metadata
  filter(!strain %in% c("sll1906-asRNA", "trxA2")) %>%
  mutate(
    setup = recode(setup, `1` = "shake flask", `2` = "photonfluxostat"),
    strain = factor(strain, unique(strain)[c(7,1:6)])
  )
...
# only gltA with the representation as µmol L-1 OD720-1 
