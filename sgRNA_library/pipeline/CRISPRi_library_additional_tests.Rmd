...
plot_fitness_ANOVA_bw <- df_ms %>% filter(timepoint == 0) %>%
  ...
  xyplot(fitness_score ~ factor(ANOVA_bin) | factor(condition) %>% 
    ...
    panel = function(x, y, ...) {
      ...
      panel.pvalue(x, y, fixed_pos = 8, std = 5, 
...
and the predicted depletion/enrichment over time in fold change (`FC_fit`). 
...
plot_mutant_mu <- lapply(c("FC", "FC_fit"), function(var) {
  xyplot(log2(get(var)) ~ timepoint | sgRNA_short, 
    ...
    groups = paste(induction, sgRNA_index), col = rep(custom_palette[c(6,3)], each = 2),
    as.table = TRUE, between = list(x = 0.3, y = 0.3), layout = c(5, 1),
    par.settings = custom.lattice, type = "l", lwd = 2,
    xlab = "time [h]", ylab = bquote(log[2] ~ "fold change"), 
    ...
    panel = function(x, y, ...) {
      ...
      panel.key(c("induced", "uninduced"), corner = c(0.1,0.05), 
        col = custom_palette[c(6,3)], points = FALSE, lines = TRUE, cex = 0.6)
    }
  )
})
...
plot_mutant_mu_2 <- lapply(list(list_calvin, list_photores, list_faster), function(genes) {
  ...
  xyplot(mutant_growth_rate/average_growth_rate*100 ~ factor(sgRNA_short) | condition, .,
    ...
    pch = 19, xlab = "", ylab = "µ [% pop. average]", 
    ylim = ylim, 
    scales = list(alternating = FALSE, x = list(rot = 30, cex = 0.7)),
    panel = function(x, y, ...) {
      panel.grid(h = -1, v = -1, col = grey(0.9))
...
df_all <- df_mu %>% ungroup %>% 
  ...
  select(
    -FC, -Gene.ontology.IDs, -Process.abbr, -Pathway.abbr,
    -baseMean, -lfcSE, -stat, -pvalue, -Gene.names, -Length, 
    -MolWeight, -location, -start_bp, -end_bp, -Process, -Pathway
  )
```
...
```{r, message = FALSE}
# and save to disk
write_csv(df_all, "../processed_data/Supplemental_data_S4.csv")
```


2. Selected genes for photorespiation, Calvin cycle, and increased growth rate.
   This is **Supplemental Data S6**.
...
df_all %>% ungroup %>% 
  ...
  write_csv("../processed_data/Supplemental_data_S6.csv")
```
...
plot_volcanoplot <- df_rnaseq %>%
  ...
  xyplot(-log10(padj) ~ setNames(log2FoldChange, Name) | 
    ...
    cex = 0.6, pch = 19, col = c(grey(0.5, 0.4), "#E7298A"),
    scales = list(alternating = FALSE), 
    ...
    panel = function(x, y, ...) {
      ...
      panel.text(x[sel], y[sel]+5, labels = names(x[sel]), cex = 0.6, col = "#E7298A")
      panel.key(c("not significant", "significant"), col = c(grey(0.6), "#E7298A"),
...
The next step is looking at interesting candidates that can potentially have an effect on
growth rate. Known examples for genes with positive correlation with growth 
...
Save list of genes with significant change in at least one of four conditions 
(CRISPRi repression mutants) as **Supplemental Data S7**.


```{r, message = FALSE}
write_csv(df_sig_genes, "../processed_data/Supplemental_data_S7.csv")
```
...
plot_dots <- xyplot(factor(Process.abbr) ~ setNames(log2FoldChange, locus) | 
    ...
    xlab = bquote(log[2] ~ "fold change"), ylab = "",
    par.settings = custom.lattice, layout = c(4, 1),
    ...
    panel = function(x, y, subscripts, ...) {
      ...
      cols = colorRampPalette(rev(RColorBrewer::brewer.pal(5, "PuOr")))(20)[round(cols)]
...
df_lac_prod <- bind_rows(.id = "setup",
    read_csv("../raw_data/lactate_measurement_batch.csv"),
    read_csv("../raw_data/lactate_measurement_photofluxostat.csv")
  ) %>%

  # add µmol/L*OD720 and per gDCW (using an average value of 0.4 gDCW/OD720)
  mutate(
    lactate_µmol_L_OD = lactate_µmol_L / OD_720,
    lactate_µmol_L_gDCW = lactate_µmol_L_OD * 0.4
  ) %>%
  
  # filter strains and adjust metadata
  filter(strain != "trxA2") %>%
  mutate(
    setup = recode(setup, `1` = "shake flask", `2` = "photonfluxostat"),
    strain = gsub("_[0-9]*$", "", strain) %>% factor(., unique(.))
  )
...
# change color scheme to black for NT0 control
custom_lattice <- custom.lattice()
custom_lattice$superpose.symbol$col <- RColorBrewer::brewer.pal(8, "Dark2")[c(8,4,5,6,3,2,4,5)]


# dot plot
plot_lac_prod <- lapply(1:4, function(i) {
  
  xyplot(get(combinations$unit[i]) ~ factor(time_h), 
    filter(df_lac_prod, setup == combinations$setup[i]),
    ...
    panel = function(x, y, groups, subscripts) {
      ...
      panel.errbars(x, y, groups = groups, subscripts = subscripts,
        ewidth = 0, type = "l", lwd = 2)
      if (i %in% c(1, 3)) {
        panel.key(points = FALSE, cex = 0.7, groups = groups, subscripts = subscripts)
      }
    }
  )
})

# alternatively a set of barplots for the same data
plot_lac_prod <- lapply(1:4, function(i) {
  
  xyplot(get(combinations$unit[i]) ~ factor(time_h), 
    filter(df_lac_prod, setup == combinations$setup[i], time_h != 0),
    groups = strain, as.table = TRUE, lwd = 2,
    xlab = ifelse(i %in% c(3, 4), "time [h]", ""), 
    ylab = ifelse(i %in% c(1, 3), "µmol L-1", "µmol L-1 OD720-1"),
    par.settings = custom_lattice,
    scales = list(alternating = FALSE),
    panel = function(x, y, ...) {
      ...
      panel.xyplot(x, y, pch = 19, alpha = 0.4, ...)
      if (i %in% c(1, 3)) {
        panel.key(points = FALSE, cex = 0.7, ...)
      }
    }
  )
})

# conduct a t-test for the last time point
# only gltA and pcnB with the representation as µmol L-1 OD720-1 
# is significant (p <= 0.05)
xyplot(lactate_µmol_L_OD ~ factor(strain) | setup, 
  filter(df_lac_prod, time_h %in% c(30, 48)),
  groups = strain, layout = c(1,2),
  par.settings = custom_lattice, pch = 19,
  scales = list(alternating = FALSE, x = list(rot = 35)),
  panel = function(x, y, ...) {
    panel.grid(h = -1, v = -1, col = grey(0.9))
    panel.errbars(x, y, ewidth = 0, ...)
    panel.pvalue(x, y, std = "NT0", verbose = TRUE, 
      col = grey(0.3), ...)
  }
)
...
svg("../figures/main/Figure6_bars.svg", width = 5.5, height = 5.5)
print(plot_lac_prod[[1]], position = c(0,0.49,0.53,1), more = TRUE)
print(plot_lac_prod[[2]], position = c(0.47,0.49,1,1), more = TRUE)
print(plot_lac_prod[[3]], position = c(0.015,0,0.53,0.53), more = TRUE)
print(plot_lac_prod[[4]], position = c(0.47,0,1,0.53))
