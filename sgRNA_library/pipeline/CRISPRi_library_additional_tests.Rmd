...
df_sig_genes %>% filter(!duplicated(locus)) %>% select(locus) %>%
  write_csv("/home/michael/Desktop/string_list.csv")
```
...
plot_dots <- xyplot(factor(Process.abbr) ~ setNames(log2FoldChange, Gene.names) | 
    ...
    panel = function(x, y, ...) {
      ...
      panel.text(x[sel], y[sel], labels = names(x)[sel], cex = 0.6, col = grey(0.3),
        pos = 3, offset = -2)
    }
  )
...
df_sig_genes %>% 
  ...
  barchart(factor(Process.abbr) ~ n_upreg_genes + -1*n_downreg_genes | 
    ...
    as.table = TRUE, xlab = "diff. expressed genes (-1 > log2 FC > 1)", ylab = "",
...
```{r, fig.width = 6.5, fig.height = 7.3}
#cowplot::ggdraw() + cowplot::draw_plot(plot_clusters, 0.011, 0.5, 0.91, 0.15)
print(plot_volcanoplot, position = c(0,0.64,1,1), more = TRUE)
print(plot_heatmap, position = c(0.037,0.50,0.987,0.67), more = TRUE)
print(plot_dots, position = c(0,0,1,0.55), more = TRUE)
...
```{r, include = FALSE, eval = FALSE}
svg("../figures/main/Figure4.svg", width = 6.5, height = 7.3)
print(plot_volcanoplot, position = c(0,0.64,1,1), more = TRUE)
print(plot_heatmap, position = c(0.037,0.50,0.987,0.67), more = TRUE)
print(plot_dots, position = c(0,0,1,0.55), more = TRUE)
dev.off()
