...
df_rnaseq %>% filter(Name %in% sig_genes) %>%
...
One of those is network interactions analysis of sig. reguated genes, and the 
other is simply looking at interesting cadndidates that alone can have an effect.
...
```{r}
# As before, we merge clusters 1 and 3 (significant for pmgA) and
# clusters 2 and 4 (significant for others) and prepare names for
# StringDB
clusters %>%
  plyr::mapvalues(c(3,4), c(1,2)) %>%
  enframe(name = "Name", value = "cluster") %>%
  filter(!grepl("SGL_", Name)) %>%
  left_join(df_rnaseq %>% select(Name, Pathway, Protein)) %>%
  filter(!duplicated(Name)) %>%
  arrange(cluster, Pathway) %>%
  write_csv("../processed_data/RNAseq_significant_genes.csv")
```
