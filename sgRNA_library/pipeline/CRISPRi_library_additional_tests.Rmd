...
plot_volcanoplot <- df_rnaseq %>%
  ...
  filter(!grepl("MYO\\_", Name)) %>%
  
  xyplot(-log10(padj) ~ setNames(log2FoldChange, Name) | 
      Condition %>% factor(., unique(.)[c(2,3,4,5,1)]), ., 
    as.table = TRUE, groups = Significance,
    xlab = "log2 FC", ylab = "-log10(p value)",
    par.settings = custom.lattice, layout = c(5, 1),
    # "#00526D" "#CC476B" "#9F7000" "#228B00" "#009681" "#0082CE" "#B646C7"
    cex = 0.6, pch = 19, col = c(grey(0.5, 0.4), "#00526D"),
    scales = list(alternating = FALSE),
    ylim = c(-5, 55), xlim = c(-5, 5),
    panel = function(x, y, ...) {
      panel.grid(h = -1, v = -1, col = grey(0.9))
      panel.xyplot(x, y, ...)
      # select top diff expr genes and plot labels
      sel <- which(!between(x, -1.3, 1.3) & y >= 12)
...
sig_genes <- df_rnaseq %>% filter(!between(log2FoldChange, -1, 1), padj <= 0.05) %>%
  pull(Name) %>% unique
...
sig_genes_mat <- df_rnaseq %>% filter(Name %in% sig_genes) %>%
  ...
  select(Name, pmgA, sll1969, slr1340, slr1916, ssl2982) %>%
...
four genes, but pmgA has a different if not even inverted expression pattern.
This could be an outlier as the samples were prepared/sequenced at a different date.
...
plot_splom <- df_rnaseq %>% filter(Name %in% sig_genes) %>%
...
Three to four clusters seems to be the consensus optimal number of clusters.
A number of four clusters were chosen covering the two up and down-regulated clusters
for pmgA and the other 4 genes.
...
clusters <- sig_genes_mat %>% dist %>% 
  hclust(method = "ward.D2") %>% cutree(k = 4)

    plot_clusters <- fviz_dend(sig_genes_mat %>% dist %>%
  hclust(method = "ward.D2"), k = 4, main = "",
  k_colors = custom.lattice()$superpose.symbol$col[1:4],
  show_labels = FALSE
)
...
# 3. Trying pathway enrichment for specific clusters 1-4
# alternatively to testing 1 cluster at a time, cluster 1+3 (pmgA-specific)
# and cluster 2+4 (ssl2982 and slr1916 specific) can be combined
kegga_result <- clusters %>% plyr::mapvalues(c(3,4), c(1,2)) %>%
...
# plot the resut
kegga_result %>% select(Pathway, P.1:P.2) %>% rename(pmgA = P.1, other = P.2) %>%
...
# 1-4 to be used for String DB, or simple heatmap plotting
df_sig_genes <- clusters %>%
  enframe(name = "locus", value = "cluster") %>%
  filter(!grepl("SGL|MYO", locus)) %>%
  ...
  left_join(df_ms %>% ungroup %>%
    select(locus, Process, Process.abbr, Gene.names) %>%
    filter(!duplicated(locus))) %>%
  
  arrange(cluster, Process, Condition, log2FoldChange)

write_csv(df_sig_genes, "../processed_data/RNAseq_significant_genes.csv")
```


```{r, message = FALSE, include = FALSE, eval = FALSE}
# extracting a gene lsit for string DB
df_sig_genes %>% filter(cluster %in% c(2,4), !duplicated(locus)) %>% 
  select(locus) %>% write_csv("/home/michael/Desktop/string_list.csv")
```
...
Here the purpose is simply to plot the most interesting genes as heatmap
...
# and annotated genes
plot_sig_genes <- lapply(c(2,4), function(clust) {
...
df_sig_genes %>% 
  ...
  # omit pmgA from data
  filter(cluster %in% c(2,4), Condition != "pmgA") %>%
  
  xyplot(factor(Process.abbr) ~ log2FoldChange | Condition %>% factor(., unique(.)[c(2:5,1)]), .,
    as.table = TRUE,
    xlab = "log2 FC", ylab = "Pathway",
    par.settings = custom.lattice, layout = c(4, 1),
    scales = list(alternating = FALSE), xlim = c(-4, 4),
    panel = function(x, y, ...) {
      panel.grid(h = -1, v = -1, col = grey(0.9))
      ...
      panel.xyplot(x, y, # jitter.data = TRUE, horizontal = TRUE, amount = 0.2
        pch = 19, cex = 0.6+(0.4*abs(x)), col = cols)
    }
  )
```
...
df_sig_genes %>% 
  ...
  barchart(factor(Process.abbr) ~ n_upreg_genes + -1*n_downreg_genes | 
    Condition %>% factor(., unique(.)[c(2:5,1)]), .,
    col = c("darkorange", "steelblue"), border = NA,
    as.table = TRUE, xlab = "diff. expressed genes (-1 > log2 FC > 1)", ylab = "Pathway",
    par.settings = custom.lattice, layout = c(4, 1),
    scales = list(alternating = FALSE),
    panel = function(x, y, ...) {
      panel.grid(h = -1, v = -1, col = grey(0.9))
...
#print(plot_splom, position = c(0.53,0.45,1.0,0.75), more = TRUE)
cowplot::ggdraw() + cowplot::draw_plot(plot_clusters, 0.01, 0.57, 0.93, 0.14)
print(plot_volcanoplot, position = c(0,0.67,1,1), more = TRUE)
print(plot_heatmap, position = c(0,0.45,1,0.62), more = TRUE)
print(plot_sig_genes[[1]], position = c(0,0.20,1,0.49), more = TRUE)
print(plot_sig_genes[[2]], position = c(0,-0.02,1,0.27), more = TRUE)
```
...
library was grown before. The transcriptome of the cells was analyzed using RNA-Seq.
**A.** Volcanoplot of log<sub>2</sub> fold change (log<sub>2</sub> FC) against
adjusted p-value for each gene. Grey - non-significantly different genes, blue - 
significantly different genes (threshold: negative log<sub>10</sub> p-value $\geq$ 2, 
-1 $\geq$ log<sub>2</sub> FC $\geq$ 1).
**B.** Heat map of all 430 significantly different genes for the five selected mutants.
The genes were clustered into four different groups based on similarity of gene expression.
**C.** To be added.
**D.** To be added.


```{r, include = FALSE, eval = FALSE}
svg("../figures/main/Figure4.svg", width = 8, height = 8)
cowplot::ggdraw() + cowplot::draw_plot(plot_clusters, 0.01, 0.57, 0.93, 0.14)
print(plot_volcanoplot, position = c(0,0.67,1,1), more = TRUE)
print(plot_heatmap, position = c(0,0.45,1,0.62), more = TRUE)
print(plot_sig_genes[[1]], position = c(0,0.20,1,0.49), more = TRUE)
